...
// UserID: Cek0037
// Filename: project4_Kirkconnell_Cek0037.cpp
// Compile: VisualStudio
// I used StackOverflow to learn how to use linked data structures
// and implement unit testing.
// Description: This program allows players to create their trivia
// questions and answers. Multiple questions are organized and 
// managed using a linked data structure. The program asks a question
// to the player, inputs the player's answer, and checks if the 
// player's answer matches the actual answer. If it matches,
// the player is awarded points for that question. If the player
// enters the wrong answer, the program displays the correct answer.
// When all the questions have been asked, the total award points
// that the player has won should be displayed.
...
struct Trivia {
    ...
    // Prompts user to add a question
    void add_question() {
        string new_question;
        string new_answer;
    ...
    // Asks a certain number of questions
    ...
    // Initializes a list with example questions. Appropriate versions are 0 and 1
    void initial_questions(int version) {
        size = 0;
        score = 0;
        if (version > -1) {  // Version 0 selects this alone
            add("How long was the shortest war on record? (Hint: how many minutes)", "38", 100);
        }
        if (version > 0) {  // Version 1 adds this section and the one before
